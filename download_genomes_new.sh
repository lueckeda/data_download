...
cd $SCRATCHnew # for using on HPCC
...
if [ ! -d data_downloads ]; then
    ...
    echo "created directory /mnt/gs18/scratch/users/$USER/data_downloads"
fi
cd data_downloads
...
get_data () {
    if [[ ! -d $1 ]]; then
        mkdir $1
        echo "created directory /mnt/gs18/scratch/users/$USER/data_downloads/$1/"
    fi
    cd $1
    [[ $GENOME == 'y' ]] && eval sbatch $2GetGenome.sb
    [[ $TRANSCRIPTOME == 'y' ]] && eval sbatch $2GetTranscriptome.sb
    [[ $PROTEOME == 'y' ]] && eval sbatch $2GetProteome.sb
    cd ..
}
